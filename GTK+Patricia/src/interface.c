...
void on_button_busca_pat_clicked (GtkWidget * widget, gpointer data){ 
   gtk_stack_set_visible_child_name(stack, "view_pesq_pat");
}

void on_button_imprimir_TST_clicked (GtkWidget * widget, gpointer data){ 
   gtk_stack_set_visible_child_name(stack, "view_str_tst");
}

void on_button_imprimir_dic_clicked (GtkWidget * widget, gpointer data){ 
   gtk_stack_set_visible_child_name(stack, "view_dic");
}
...
void insertDic(FILE* file){
    ...
    printTstOrd(&rootTst, list_view);
    if(file == NULL){
        return;
    }

    do{
        result = fgets(str, 20, file);
        str[strlen(str)-1] = '\0';
        add_to_list(tst_view, str, 0);
        insertTst(&rootTst, str, 0);
    }while(!feof(file));
   
}


void printTstOrd(apointerTst* rootTst, GtkWidget* list_tst){
    if((*rootTst) == NULL){
        return;
    }
    if ((*rootTst)->left != NULL || (*rootTst)->right != NULL){
        printTstOrd(&(*rootTst)->left, list_tst);
        printTstOrd(&(*rootTst)->center, list_tst);
        printTstOrd(&(*rootTst)->right, list_tst);
    }
    g_print("%s\n", (*rootTst)->key);
